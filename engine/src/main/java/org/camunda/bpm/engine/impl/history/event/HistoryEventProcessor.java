...
public class HistoryEventProcessor {
   ...
   */
  public interface HistoryEventCreator {
    /**
   ...
   * of the creation of different HistoryEvents.
   */
  public interface HistoryEventsCreator {
    /**
     * Creates the {@link HistoryEvent}s with the help off the given
     * {@link HistoryEventProducer}.
     *
     * @param producer the producer which is used for the creation
     * @return the created {@link HistoryEvent}s
     */
   ...
   */
  public static void processHistoryEvent(HistoryEventCreator creator) {
    processingHistoryEvents(creator, null);
  }


  /**
   * Process {@link HistoryEvent}s and handles them directly after creation.
   * The {@link HistoryEvent}s are created with the help of the given
   * {@link HistoryEventsCreator} implementation.
   *
   * @param creator the creator is used to create the {@link HistoryEvent}s which should be thrown
   */
  public static void processHistoryEvents(HistoryEventsCreator creator) {
    processingHistoryEvents(null, creator);
  }

  private static void processingHistoryEvents(HistoryEventCreator eventCreator, HistoryEventsCreator eventsCreator) {
    ...
    if (eventCreator != null) {
      HistoryEvent historyEvent = eventCreator.createHistoryEvent(historyEventProducer);
      historyEventHandler.handleEvent(historyEvent);
    } else if (eventsCreator != null) {
      List<HistoryEvent> historyEvents = eventsCreator.createHistoryEvents(historyEventProducer);
      historyEventHandler.handleEvents(historyEvents);
    }
  }
}
